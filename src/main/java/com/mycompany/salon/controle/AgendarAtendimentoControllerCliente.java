 ...
 */
@Named
public class AgendarAtendimentoControllerCliente implements Command, Serializable {
    ...
    public void execute(HttpServletRequest req, HttpServletResponse res) {
        ...
        if (req.getParameter("opcao").equals("") || req.getParameter("cliente").equals("") || req.getParameter("data").equals("")) {
            try {
                res.sendRedirect("agendamentoDeHorarioCliente.jsp?msg=Preencha os campos vazios.");
            ...
            try {
                res.sendRedirect("agendamentoDeHorarioCliente.jsp?msg= Ja existe um atendimento marcado para esse horario e data com esse atendente.");
            } catch (IOException ex) {
                Logger.getLogger(AgendarAtendimentoControllerCliente.class.getName()).log(Level.SEVERE, null, ex);
            }
        } else {
            if (atendimentoDao.createAtendimento(atendimento)) {
                ...
                try {
                    res.sendRedirect("agendamentoDeHorarioCliente.jsp?msg=Nao foi possivel marcar o horario.");
                } catch (IOException ex) {
                    Logger.getLogger(AgendarAtendimentoControllerCliente.class.getName()).log(Level.SEVERE, null, ex);
                }
            }
